...
onAuthStateChanged(auth, async (user) => {
    if (user) {
        ...
        // Hide loader and login, show app
        if(loader) {
        ...
        // No user: Hide loader, show login
...
btnLogin.addEventListener('touchend', handleLogin);
...
function initListeners() {
    ...
    onSnapshot(collection(db, 'artifacts', appId, 'presence'), (snap) => {
        ...
        snap.forEach(d => {
            const data = d.data();
            // Check if timestamp exists and is within last 3 minutes (allowing for latency)
            if (data.timestamp && data.timestamp.toMillis) {
                const diff = now - data.timestamp.toMillis();
                if (diff < 180000) count++; // 3 minutes TTL
            }
        });
        document.getElementById('viewer-number').innerText = count;
    });

    // THIS ONE IS IMPORTANT: Listen to ALL schedule items, don't filter in snapshot
    onSnapshot(collection(db, 'artifacts', appId, 'public', 'data', 'schedule'), (snap) => {
        allScheduleItems = []; // Reset global store
        snap.forEach(d => {
            const data = d.data();
            // We store EVERYTHING. Filtering happens in the Loop based on current time/day
            allScheduleItems.push({id: d.id, ...data});
        });
        renderScheduleSidebar(); // Updates visual list (filtered for 'today')
    });
...
function checkScheduleLoop() {
    ...
    // Play or Standby
    if (nextActiveItem) {
        playProgram(nextActiveItem, timeOffset);
    } else {
        goStandby();
    }
...
function updateTickerVisibility() {
     ...
     if(!localTickerEnabled) {
         tickerEl.classList.remove('active');
         currentTickerMsg = ""; // Reset logic
     }
}


// 4. Player Logic (UPDATED)
function playProgram(item, offsetSeconds) {
...
function loadStream(url, startTimeSeconds) {
    ...
    const onReady = () => {
        videoElement.currentTime = startTimeSeconds; // Use passed seconds
...
window.openProgramDetails = (id) => {
    ...
    const item = allScheduleItems.find(p => p.id === id);
    if(!item) return;
...
function renderScheduleSidebar() {
    ...
    const todaysItems = allScheduleItems.filter(i => String(i.day) === String(today));
